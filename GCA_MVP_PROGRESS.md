...
| DT-038 | Major | Compartimentalização — notificação cross-project | `_notify_admins_questionnaire_submitted` envia pra TODOS admins ativos (`is_admin=true AND is_active=true`). Projeto A comunica existência + nome + GP email a admin que pode não ter relação com ele. Numa única instância com 1 admin é aceitável; com instância futura multi-admin por área quebra compartimentalização §2.2. | Auditoria 2026-04-18 | Aberto |
