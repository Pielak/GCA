...
| ~~**DT-081** (RESOLVIDA pelo MVP 32, commit `506e1c0`+`80851d3`)~~: OCGUpdaterService fallback `_load_persona_scores` quebrado + prompt sub-ótimo | `ocg_updater_service.py` | ✅ **RESOLVIDA** — `_load_persona_scores` reescrito para ler de `ocg_individual` (cumulativo MVP 31). Normalização `persona_tag.lower()` antes do lookup em `PERSONA_TO_PILLAR`. `arguider_compactor.py` reduz payload n8n de ~217KB para ~7.8KB (96.4%). OCG agora acumula em produção (`ocg.status='active'`). |
