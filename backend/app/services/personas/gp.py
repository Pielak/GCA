...
class GPPersona(Persona):
    ...
    async def analyze(
        ...
        try:
            ...
            logger.exception("gp.llm_call_failed", error=str(e))
        ...
        if meta.total_failure or meta.level >= 1:
            logger.error("gp.parse_failed", level=meta.level, warnings=meta.warnings)
            return self._fallback_output(passada=passada)
        if meta.level > 0:
            logger.warning("gp.parse_repaired", level=meta.level, warnings=meta.warnings)
