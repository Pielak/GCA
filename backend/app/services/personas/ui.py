...
class UIPersona(Persona):
    ...
    async def analyze(
        ...
        try:
            ...
            logger.exception("ui.llm_call_failed", error=str(e))
        ...
        if meta.total_failure or meta.level >= 1:
            logger.error("ui.parse_failed", level=meta.level, warnings=meta.warnings)
            return self._fallback_output(passada=passada)
        if meta.level > 0:
            logger.warning("ui.parse_repaired", level=meta.level, warnings=meta.warnings)
