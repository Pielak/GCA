...
- **Fase 9.2 — Detalhamento on-demand por item (IA local, §6.2 baixa criticidade):** endpoint `GET /projects/{id}/modules/{mid}/details` invoca modelo local (Ollama) para gerar, sob demanda: `what_it_is` (descrição técnica curta), `prerequisites` (pré-requisitos), `missing_inputs` (informações que o Arguidor precisa ter recebido na Ingestão para este item ser elaborado), `input_examples` (exemplos de doc/trecho que viabilizam). Cache persistido em `module_candidates.details_json` para não regenerar a cada clique. Modal na UI do Roadmap ao clicar no item.
...
- **Fase 9.5 — Ponte item → ingestão guiada:** quando item está `needs_input`, modal tem botão "Prover informações": (a) questionário complementar com as perguntas geradas na Fase 9.3; (b) upload de documento adicional marcado com `target_module_id`. Pipeline reprocessa o item e atualiza `readiness_status`. Ao virar `ready_for_codegen`, botão "Gerar código" aparece (alinha com MVP 3 §7).

#### Regras duras

- Nenhum item vai para CodeGen sem `readiness_status='ready_for_codegen'` aprovado explicitamente pelo GP. Afirmação binária (§ política de linguagem binária) — não existe "meio pronto".
- Cada chamada de IA desta fase grava provedor, modelo, criticidade e custo (§6.3).
- IA local nunca decide sozinha arquitetura, DAG ou readiness — texto explicativo é baixa criticidade; dependências e readiness são alta criticidade e exigem premium (§6.3).
