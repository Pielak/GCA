...
export const BLOCKS: BlockDef[] = [
  {
    ...
    questions: [
      ...
      {
        id: 'deliverable_type',
        label: 'Tipo de entregável (obrigatório)',
        help: 'Classificação bloqueante do projeto. Define quais agentes serão ativados, quais artefatos são obrigatórios, e quais critérios de billing e métricas serão usados. Sem esta definição, o projeto não avança.',
        type: 'single',
        options: [
          'Sistema web novo',
          'Aplicação mobile',
          'Módulo funcional / Extensão de ecossistema',
          'Melhoria em sistema existente',
          'Integração com sistema legado',
          'Modernização / Refatoração',
      ...
      {
        id: '4',
        label: 'Detalhamento da iniciativa',
        help: 'Selecione uma ou mais categorias adicionais que melhor descrevem o que o projeto pretende entregar. Isso ajuda os agentes de IA a calibrar a análise de escopo.',
        type: 'multi',
  ...
  },
  {
    ...
    questions: [
      { id: '15', label: 'Entregável principal do projeto', help: 'O que o projeto vai gerar como produto final? Pode ser mais de um.', type: 'multi', options: ['Executável desktop', 'Aplicação web', 'API', 'Microserviço', 'Aplicativo mobile', 'Dashboard', 'Job/Worker', 'CLI', 'Biblioteca/SDK'], allowNA: true },
      ...
      { id: '17', label: 'Modelo de execução', help: 'Onde o sistema vai rodar? "Stand-alone" = máquina local. "Cloud" = nuvem. "Containerizado" = Docker/Kubernetes. Pode selecionar mais de um.', type: 'multi', options: ['Stand-alone', 'On-premises', 'Cloud', 'Híbrido', 'Containerizado', 'Offline com sincronização posterior'], allowNA: true },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: '22', label: 'Tipo de frontend', help: 'SPA = página única (Gmail). SSR = renderização no servidor. PWA = funciona offline.', type: 'multi', options: ['Web SPA', 'SSR', 'PWA', 'Desktop UI', 'Mobile app', 'Painel administrativo', 'Portal autenticado'], allowNA: true, linkedTo: { question: '21', value: 'Não', message: 'Projeto sem frontend (Q21).' } },
      { id: '23', label: 'Stack de frontend preferida', help: '"Sem preferência" permite que o GCA recomende a melhor opção.', type: 'multi', options: ['React', 'Vue', 'Angular', 'Next.js', 'Vite + React', 'Electron', 'Flutter', 'React Native', 'Sem preferência'], allowNA: true, linkedTo: { question: '21', value: 'Não', message: 'Projeto sem frontend (Q21).' } },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: '28', label: 'Framework backend preferido', help: 'FastAPI/Django = Python. NestJS/Express = Node.js. "Sem preferência" = GCA recomenda.', type: 'multi', options: ['FastAPI', 'Django', 'Flask', 'NestJS', 'Express', 'Spring Boot', 'ASP.NET', 'Quarkus', 'Sem preferência'], allowNA: true, linkedTo: { question: '26', value: 'Não', message: 'Projeto sem backend (Q26).' } },
      { id: '29', label: 'Tipo de backend', help: '"REST API" = mais comum. "GraphQL" = consultas flexíveis. "WebSocket" = tempo real.', type: 'multi', options: ['REST API', 'GraphQL', 'gRPC', 'WebSocket', 'Batch', 'Worker', 'BFF', 'Misto'], allowNA: true, linkedTo: { question: '26', value: 'Não', message: 'Projeto sem backend (Q26).' } },
  ...
  },
  {
    ...
    questions: [
      { id: '31', label: 'Banco de dados principal', help: 'PostgreSQL é o mais versátil e recomendado para a maioria dos casos.', type: 'single', options: ['PostgreSQL', 'MySQL', 'SQL Server', 'Oracle', 'MongoDB', 'SQLite', 'Sem preferência', NA_VALUE] },
      { id: '32', label: 'Perfil de uso do banco de dados', help: '"Transacional" = muitas escritas/leituras (e-commerce). "Analítico" = relatórios complexos. "Documental" = dados JSON.', type: 'multi', options: ['Transacional', 'Analítico', 'Documental', 'Catálogo', 'Event store', 'Misto'], allowNA: true },
      ...
      { id: '34', label: 'Finalidade do Redis', help: 'Para que o Redis será usado? Pode selecionar múltiplas finalidades.', type: 'multi', options: ['Cache de leitura', 'Sessões', 'Rate limiting', 'Pub/Sub', 'Locks distribuídos', 'Filas leves'], allowNA: true, linkedTo: { question: '33', value: 'Não', message: 'Redis não necessário (Q33).' } },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: '40', label: 'Finalidade da IA no projeto', help: '"Geração de código" = IA escreve código. "Chat assistivo" = assistente inteligente.', type: 'multi', options: ['Análise de requisitos', 'Geração de código', 'Documentação técnica', 'Documentação negocial', 'Revisão de código', 'Testes automatizados', 'Classificação de artefatos', 'Chat assistivo'], allowNA: true, linkedTo: { question: '39', value: 'Não', message: 'IA não utilizada (Q39).' } },
      { id: '41', label: 'Provedor de IA preferencial', help: '"Anthropic" = Claude. "OpenAI" = GPT. "Sem preferência" = GCA recomenda.', type: 'multi', options: ['Anthropic', 'OpenAI', 'Gemini', 'DeepSeek', 'Grok', 'Outro', 'Sem preferência'], allowNA: true, linkedTo: { question: '39', value: 'Não', message: 'IA não utilizada (Q39).' } },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: '48', label: 'Entregáveis esperados do pipeline GCA', help: 'O que o GCA deve gerar? Documentos técnicos, sugestões de arquitetura, planos de teste, etc.', type: 'multi', options: ['Sugestão de arquitetura', 'Sugestão de stack tecnológico', 'Documento técnico consolidado', 'Documento negocial consolidado', 'Gap analysis (análise de lacunas)', 'Backlog inicial sugerido', 'Plano de testes', 'Plano de segurança', 'Plano de observabilidade', 'Plano de deploy'], allowNA: true },
      { id: '49', label: 'Formato de retorno desejado', help: '"Painel no GCA" = visualização direta na plataforma. Pode selecionar múltiplos.', type: 'multi', options: ['Painel no GCA', 'HTML', 'Markdown', 'DOCX', 'PDF', 'JSON estruturado', 'YAML'], allowNA: true },
    ],
  },
]

/**
 * Campos de resultado da análise (A.12).
 * NÃO são perguntas — são dados retornados pelo n8n/OCG após análise das 49 respostas.
 * Exibidos na zona de resultado, não no formulário.
 */
export const ANALYSIS_RESULT_FIELDS = {
  restrictions: { id: '50', label: 'Restrições do projeto', help: 'Restrições técnicas, regulatórias, orçamentárias ou de prazo identificadas pela análise.' },
  observations: { id: '51', label: 'Observações adicionais', help: 'Ajustes necessários ou dados que devem ser revistos durante o projeto.' },
  percentage: { id: '52', label: 'Percentual respondido', help: 'Percentual de questões aplicáveis respondidas. Mínimo 80% para aprovação.' },
  validators: { id: '54', label: 'Áreas que validaram', help: 'Papéis das pessoas que responderam e revisaram o questionário.' },
} as const
