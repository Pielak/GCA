...
- ❌ **Proibido erro silencioso.** Toda função, task, endpoint e processo é BINÁRIO: ✅ sucesso (com log de conclusão) OU ❌ erro explícito (com log, mensagem clara, ação esperada do usuário). Parada silenciosa = erro. Regra: `try: ... except Exception as e: logger.error(...); raise` — NUNCA `except: pass` ou swallow exceções. Se o código não pode falhar, documentar por quê (garantia do framework, validação anterior, invariante). Erro silencioso é pior que crash — crash avisa, silêncio engana. Retrabalho, debug, timeout e desconforto do usuário são custos reais do erro silencioso.
